...
const handleRegister = () => {
  if (!name.value || !email.value || !phone.value || !password.value || !confirm.value) {
    showModal('error', 'Thiếu thông tin', 'Vui lòng nhập đầy đủ thông tin trước khi đăng ký.')
    return
  }

  if (password.value !== confirm.value) {
    showModal('error', 'Mật khẩu không khớp', 'Mật khẩu xác nhận không đúng. Vui lòng kiểm tra lại.')
    return
  }

  showModal('success', 'Đăng ký thành công!', 'Tài khoản của bạn đã được tạo. Chào mừng bạn đến với VinaTech!', () => {
    router.push('/login')
  })
}
</script>
...
.left p { font-size: 13px; opacity: 0.85; }
.left img { width: 100%; border-radius: 12px; }
.right { padding: 40px; }
...
.divider { text-align: center; font-size: 12px; margin: 15px 0; color: #94a3b8; }
...
.login { text-align: center; font-size: 12px; margin-top: 15px; }
.login a { color: #2563eb; cursor: pointer; font-weight: 600; text-decoration: none; }
.login a:hover { text-decoration: underline; }
@media (max-width: 768px) {
  .box { grid-template-columns: 1fr; width: 95%; }
  .left { display: none; }
}
