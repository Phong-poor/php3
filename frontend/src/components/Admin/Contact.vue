  ...
  <div class="page">
    ...
    <div class="filter-bar">
      ...
      <div class="filter-actions">
        <button class="icon-btn"><svg viewBox="0 0 24 24" fill="none"><line x1="4" y1="6" x2="20" y2="6"/><line x1="8" y1="12" x2="16" y2="12"/><line x1="11" y1="18" x2="13" y2="18"/></svg></button>
        <button class="icon-btn" @click="resetFilter"><svg viewBox="0 0 24 24" fill="none"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg></button>
      </div>
    </div>
    ...
    <div class="table-card">
            ...
            <td @click.stop>
              <div class="actions">
                ...
                <button class="action-btn action-delete" title="Xóa" @click="deleteContact(c.id)">
                  <svg viewBox="0 0 24 24" fill="none"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>
                </button>
              </div>
            </td>
          </tr>
      ...
      <div class="pagination-row">
        <span class="page-info">Hiển thị 1-{{ filteredContacts.length }} trong số <strong>{{ contacts.length }}</strong> liên hệ</span>
          ...
          <div class="modal modal-detail" v-if="showDetail">
            ...
            <div class="modal-body">
              <div class="detail-meta">
                <div class="meta-item"><span class="meta-key">Thời gian</span><span class="meta-val">{{ selected?.time }} · {{ selected?.date }}</span></div>
                <div class="meta-item"><span class="meta-key">Trạng thái</span><span :class="['status-badge','status-'+selected?.status]">{{ statusLabel(selected?.status) }}</span></div>
                <div class="meta-item"><span class="meta-key">Phân loại</span>
                  <div style="display:flex;gap:4px;flex-wrap:wrap">
                    <span v-for="t in selected?.tags" :key="t.label" class="tag" :style="{ background: t.bg, color: t.color }">{{ t.label }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="detail-status-update">
                ...
                <div class="toggle-group">
                  <button v-for="s in statusOptions" :key="s.value"
                    ...
                    <span class="toggle-dot" :style="selected?.status === s.value ? { background: s.color } : {}"></span>
                    {{ s.label }}
                  </button>
                </div>
              </div>
          ...
          <div class="modal modal-email" v-if="showEmail">
            ...
            <div class="modal-body">
              ...
              <div class="form-group">
                ...
                <div class="template-chips">
                  <button v-for="tpl in templates" :key="tpl.label"
                    class="tpl-chip"
                    :class="{ 'tpl-chip-active': selectedTpl === tpl.label }"
              ...
              <div class="form-group">
                ...
                <textarea class="form-input form-textarea" :class="{ err: emailErrors.body }" v-model="emailForm.body" rows="7" placeholder="Nhập nội dung email tư vấn..."></textarea>
            ...
            <div class="modal-footer">
              ...
              <button class="btn-save btn-send" @click="sendEmail" :disabled="sending">
                <svg v-if="!sending" viewBox="0 0 24 24" fill="none"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
    ...
    <transition name="toast">
      <div class="toast" v-if="toast.show" :class="'toast-' + toast.type">
        <svg v-if="toast.type==='success'" viewBox="0 0 24 24" fill="none"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
...
.page { padding: 18px 22px; background: #f0f4ff; min-height: 100vh; font-family: 'Be Vietnam Pro','Segoe UI',sans-serif; display: flex; flex-direction: column; gap: 16px; position: relative; }
...
.stat-card { background: #fff; border-radius: 14px; padding: 18px; border: 1px solid #e8edf5; box-shadow: 0 2px 8px rgba(0,0,0,0.04); display: flex; flex-direction: column; gap: 4px; }
...
.stat-card-check { width: 28px; height: 28px; background: rgba(255,255,255,0.25); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: auto; }
.stat-card-check svg { width: 14px; height: 14px; stroke: #fff; stroke-width: 2.5; fill: none; }
.stat-tag { font-size: 9.5px; font-weight: 700; letter-spacing: 1px; color: rgba(255,255,255,0.65); }
...
th { padding: 11px 16px; font-size: 10.5px; font-weight: 700; color: #94a3b8; letter-spacing: 0.6px; text-align: left; white-space: nowrap; }
.table-row { border-bottom: 1px solid #f1f5f9; transition: background 0.15s; cursor: pointer; }
...
.customer-name { font-size: 13px; font-weight: 700; color: #1e293b; }
.customer-email { font-size: 11px; color: #94a3b8; margin-top: 1px; }
.customer-phone { font-size: 11px; color: #94a3b8; }
.content-cell { font-size: 13px; color: #475569; font-style: italic; max-width: 140px; }
.tag-stack { display: flex; flex-direction: column; gap: 4px; }
.tag { display: inline-block; padding: 3px 8px; border-radius: 5px; font-size: 10px; font-weight: 700; letter-spacing: 0.3px; white-space: nowrap; }
...
.action-btn { width: 28px; height: 28px; border-radius: 7px; border: 1px solid #e2e8f0; background: #fff; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.15s; }
.action-btn svg { width: 12px; height: 12px; stroke: #64748b; stroke-width: 1.8; fill: none; }
.action-btn:hover { background: #f1f5f9; }
.action-email:hover { background: #eff6ff; border-color: #bfdbfe; }
.action-email:hover svg { stroke: #2563eb; }
...
.page-btn { width: 28px; height: 28px; border-radius: 7px; border: 1px solid #e2e8f0; background: #fff; font-size: 12px; color: #475569; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.15s; }
.page-btn:hover { background: #f1f5f9; }
.page-btn.active { background: #4f46e5; border-color: #4f46e5; color: #fff; font-weight: 700; }
.page-ellipsis { font-size: 12px; color: #94a3b8; padding: 0 4px; }

/* FAB */
.fab { position: fixed; bottom: 28px; right: 28px; width: 50px; height: 50px; border-radius: 50%; background: linear-gradient(135deg, #4f46e5, #6366f1); border: none; display: flex; align-items: center; justify-content: center; cursor: pointer; box-shadow: 0 6px 20px rgba(79,70,229,0.4); transition: transform 0.2s; z-index: 100; }
...
.modal { background: #fff; border-radius: 20px; width: 100%; box-shadow: 0 24px 60px rgba(0,0,0,0.18); display: flex; flex-direction: column; max-height: 90vh; overflow: hidden; }
.modal-detail { max-width: 520px; }
.modal-email  { max-width: 580px; }
.modal-header { display: flex; align-items: center; justify-content: space-between; padding: 18px 22px 16px; border-bottom: 1px solid #f1f5f9; gap: 12px; }
...
.icon-email { background: linear-gradient(135deg, #2563eb, #3b82f6); }
...
.modal-footer { display: flex; align-items: center; justify-content: flex-end; gap: 10px; padding: 13px 22px; border-top: 1px solid #f1f5f9; background: #fafbff; }

/* Detail modal */
.detail-meta { display: flex; flex-direction: column; gap: 10px; background: #f8faff; border-radius: 12px; padding: 14px 16px; border: 1px solid #e8edf5; }
...
.toggle-btn { display: flex; align-items: center; gap: 6px; padding: 7px 14px; border-radius: 9px; border: 1.5px solid #e2e8f0; background: #f8fafc; font-size: 12.5px; font-weight: 500; color: #64748b; cursor: pointer; transition: all 0.15s; font-family: inherit; }
...
.tpl-chip { padding: 6px 12px; border-radius: 20px; border: 1.5px solid #e2e8f0; background: #f8fafc; font-size: 12px; font-weight: 500; color: #475569; cursor: pointer; transition: all 0.15s; font-family: inherit; }
...
.btn-save { display: flex; align-items: center; gap: 7px; padding: 8px 18px; border-radius: 9px; border: none; background: linear-gradient(135deg, #4f46e5, #6366f1); color: #fff; font-size: 13px; font-weight: 600; cursor: pointer; box-shadow: 0 4px 12px rgba(79,70,229,0.3); transition: transform 0.15s; font-family: inherit; }
...
.btn-primary-sm { display: flex; align-items: center; gap: 6px; padding: 7px 14px; border-radius: 9px; border: none; background: linear-gradient(135deg, #2563eb, #3b82f6); color: #fff; font-size: 12.5px; font-weight: 600; cursor: pointer; font-family: inherit; transition: transform 0.15s; }
.btn-primary-sm:hover { transform: translateY(-1px); }
.btn-primary-sm svg { width: 13px; height: 13px; stroke: #fff; stroke-width: 2; fill: none; }
...
/* TOAST */
.toast { position: fixed; bottom: 90px; right: 28px; display: flex; align-items: center; gap: 10px; padding: 12px 18px; border-radius: 12px; font-size: 13px; font-weight: 600; z-index: 99999; box-shadow: 0 8px 24px rgba(0,0,0,0.15); }
